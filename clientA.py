...
# Simulate the scenario where Proposer A wins (A's proposal already accepted)
def simulate_scenario_a():
    print("Proposer A sending proposal...")
    send_propose_request(node_id=1, proposal_number=1, value="value_A")
    
    # Add delay to simulate some time between proposers
    time.sleep(2)
    
    print("Proposer B sending proposal after Proposer A...")
    send_propose_request(node_id=2, proposal_number=1, value="value_B")

# Call the simulation function for Scenario A
simulate_scenario_a()